/**
 * Created by liutongtong on 11/20/14.
 */
...
function uc_readyStateChanged_auth() {
    if (xmlhttp.readyState==4)
    {// 4 = "loaded"
        if (xmlhttp.status==200)
        {// 200 = OK
...
function uc_submit2auththu(openid, timestamp) {
    if (checkUsername() & checkPassword()) {
        disableAll(true);
        showLoading(true);
        ...
        document.getElementById('inputPassword').value = secret;
        for (i = 0, len = elems.length; i < len; ++i) {
            params += '&' + elems[i].name + '=' + encodeURIComponent(elems[i].value);
        }
        ...
        xmlhttp.open('POST', url, true);
        xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlhttp.onreadystatechange = uc_readyStateChanged_auth;
        xmlhttp.send(params);
        document.getElementById('inputPassword').value = '';
    }
    return false;
